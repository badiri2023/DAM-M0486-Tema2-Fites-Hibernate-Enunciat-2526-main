    ...
    */
    ...
    public static Personatge addPersonatge(String nom, Double atac, Double defensa, 
        ...
        try {
            ...
            if (tx != null) tx.rollback();
            System.out.println(" Personaje no creado : " + nom);
    ...
    public static void printPersonatges() {
        ...
        try {
            session = factory.openSession();
            var personatges = session.createQuery("FROM Personatge", Faccio.class).list();
            
            System.out.println("\nP:");
            for (Faccio f : personatges) {
                System.out.println("  - " + f.getNom() + ": " + f.getResum());
            }
        } catch (Exception e) {
            System.err.println("Error llegint Personatge: " + e.getMessage());
        } finally {
            if (session != null) session.close();
        }
    }
